...
struct inference_result *branchynet_dnn(struct inference_result *result, struct matrix *inputs, uint8_t precision, struct exit_policy *policy, struct cgr_state *policyState) {
    ...
    vector_softmax(PROBS, &logits, precision);
    result->pred = vector_argmax(&logits);
    uint16_t thresholdIndex = NUM_LABELS + ((uint16_t) result->pred);
    shouldExit = max_prob_should_exit(PROBS[result->pred], policy->thresholds[thresholdIndex]);
    #elif defined(IS_RANDOM)
    ...
    shouldExit = cgr_should_exit(PROBS[result->pred], result->pred, SCALED_EXIT_RATES[1], policy->thresholds[thresholdIndex], policy->randState, policyState, 1, NUM_OUTPUTS, precision);
    #endif
