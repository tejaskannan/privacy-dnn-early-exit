...
int32_t vector_exp_sum(struct matrix *vec, const int32_t max, const uint8_t precision) {
    if (vec->numCols != VECTOR_COLS) {
        return 0;
    }
    ...
    for (i = 0; i < vec->numRows; i++) {
        element = (int32_t) vec->data[VECTOR_INDEX(i)];
	    element = fp32_sub(element, max);
	    element = fp32_exp(element, precision);
        sum = fp32_add(sum, element);
    }
...
int32_t *vector_softmax(int32_t *result, struct matrix *vec, const uint8_t precision) {
    volatile uint8_t maxIdx = vector_argmax(vec);
    volatile int32_t max = (int32_t) vec->data[VECTOR_INDEX(maxIdx)];
    volatile int32_t sum = vector_exp_sum(vec, max, precision);

    volatile int32_t dataValue;
    volatile int32_t element;
    ...
    uint8_t i;
    for (i = 0; i < n; i++) {
        dataValue = (int32_t) vec->data[VECTOR_INDEX(i)];
	    element = fp32_sub(dataValue, max);
	    element = fp32_exp(element, precision);

        result[i] = fp32_div(element, sum, precision);
